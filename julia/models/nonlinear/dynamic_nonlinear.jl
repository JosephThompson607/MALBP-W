...
function define_dynamic_nonlinear_vars!(m::Model, instance::MALBP_W_instance)
...
function define_dynamic_nonlinear_obj!(m::Model, instance::MALBP_W_instance; same_e_cost=true)
...
function define_dynamic_nonlinear_constraints!(m::Model, instance::MALBP_W_instance)
        ...
        for t in 1:instance.n_cycles
            for s in 1:instance.equipment.n_stations
                if 1 <= t - s +1<= instance.sequences.sequence_length 
                    ...
                    @constraint(m, sum(task_time * x_wsoj[w_index,s, parse(Int,o), j] for (o, task_time) in task_times) <= sum(instance.models.cycle_time * y_lwts[l, w_index, t, s] * instance.productivity_per_worker[l] for l in 1:instance.max_workers))
                end
            end
        end
    end
...
function read_MALBP_W_solution(results_folder::String)
    ...
    #Opens the y_lwts file
    y_lwts_fp = results_folder * "y_lwts_solution.csv"
    y_lwts_df = CSV.read(y_lwts_fp, DataFrame)
    #Opens the y_w file
...
function warmstart_dynamic_nonlinear_from_md_setup!(m::Model, vars_fp::String, instance::MALBP_W_instance)
    ...
    x_soi_df, u_se_md_df, y_lwts_md_df, y_w_md_df = read_MALBP_W_solution(vars_fp)
    ...
    if !isempty(y_lwts_md_df)
        for row in eachrow(y_lwts_md_df)
            ...
            s = row.station
            value = row.value
            set_start_value(y_lwts[l,w, t, s], value)
        end
    end
...
function define_dynamic_nonlinear_redundant_constraints!(m::Model, instance::MALBP_W_instance, productivity_per_worker = Dict(1=>1., 2=>1., 3=>1., 4=>1.))
    # usesful variables
    model_indexes = [i for (i, model_dict) in instance.models.models]
    #model variables
    x_wsoj = m[:x_wsoj]
...
function define_dynamic_nonlinear!(m::Model, instance::MALBP_W_instance; preprocessing = true)
    ...
    if preprocessing   
        define_dynamic_nonlinear_redundant_constraints!(m, instance)
    end
...
function define_dynamic_nonlinear!(m::Model, instance::MALBP_W_instance, warmstart_vars_fp::String; preprocessing = true)
    ...
    define_dynamic_nonlinear_constraints!(m, instance)   
    if preprocessing   
        define_dynamic_nonlinear_redundant_constraints!(m, instance, instance.productivity_per_worker)
    end
end
