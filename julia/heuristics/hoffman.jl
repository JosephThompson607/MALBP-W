...
#This function tries to assign tasks using the hoffman heuristic, modified for workers
function modified_hoffman(instance::MALBP_W_instance; productivity_per_worker::Array{Float64}= [1., 1., 1., 1.] )
    ...
    for (model_name, model) in instance.models.models
        x_soi = recursive_task_fill(instance, model, min_workers[model_name], precedence_matrices[model_name])
        println("Model: ", model_name)
        println("min workers: ", min_workers[model_name])
        println("precedence matrix: ", precedence_matrices[model_name])
        println("x_soi: ", x_soi)   
    end
end
