...
def run_from_csv_slurm(csv_file, array_index, save_variables=False, run_time = 600, seed = None, base_file_name = 'test'):
   ...
   config = config.iloc[array_index]
   csv_name = csv_file.split('.')[0].split('/')[-1]
   print('config', config)

   with open(config['config_yaml']) as instance_f:
      conf_name = config['config_yaml'].split('.')[0].split('/')[-1]
      xp_yaml = yaml.load(instance_f, Loader=yaml.FullLoader)
      SEQUENCE_LENGTH = xp_yaml['sequence_length']
      NO_WORKERS = xp_yaml['max_workers']
      NO_STATIONS = xp_yaml['no_stations']
      WORKER_COST = xp_yaml['worker_cost']
   ...
   for milp_model in xp_yaml['milp_models']:
      ...
      if config['scenario_tree_yaml'] == "No Tree" or config['scenario_tree_yaml'] == "None":
         if seed is None:
            seed = array_index
         tree_kwargs, scenario_generator = get_scenario_generator(xp_yaml, seed)
         #Use the model mixtures from the model file
         model_mixtures = test_instance.model_mixtures
         scenario_tree_graph, final_sequences = scenario_generator(SEQUENCE_LENGTH, model_mixtures, **tree_kwargs)
         n_samples = len(final_sequences)
         tree_name = seed
         save_tree_yaml(final_sequences, model_mixtures, SEQUENCE_LENGTH, n_samples, seed, 0, file_name + 'scenario_tree', tree_name)
      else:
         with open(config['scenario_tree_yaml']) as tree_f:
            scenario_yaml = yaml.load(tree_f, Loader=yaml.FullLoader)
            final_sequences = scenario_yaml['final_sequences']
            SEQUENCE_LENGTH = scenario_yaml['sequence_length']
            model_mixtures = scenario_yaml['entry_probabilities']
            tree_name = scenario_yaml['tree_name']
      ...
      result['scenario_tree'] = config['scenario_tree_yaml']
...
def arg_parse():
   ...
   parser.add_argument('--array_index', required=False, default=None, type=int, help='index for slurm array')
...
def main_run():
   ...
   if args.xp_type == 'warm_start':
      ...
      print('running from config csv')
      run_from_csv_slurm(args.config_file, array_index=args.array_index, seed=args.seed, run_time=args.run_time, base_file_name=file_name, save_variables=args.save_variables)
   else:
